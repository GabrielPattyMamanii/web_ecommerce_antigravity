...
import { Calendar, Layers, Package, ClipboardList, Search, X, ShoppingBag } from 'lucide-react';

export function BoletasCargadas() {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <h1 className="text-3xl font-bold tracking-tight text-foreground">Boletas Cargadas</h1>
                    </div>
                    <p className="text-muted-foreground mt-1 ml-1">
                        Seleccioná una tanda para crear productos del catálogo.
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTandas.map((tanda) => (
                        <Link
                            key={tanda.nombre}
                            to={`/admin/products/boletas/${encodeURIComponent(tanda.nombre)}`}
                            className="block bg-card p-6 rounded-xl border border-border shadow-sm hover:shadow-xl hover:border-violet-500/30 transition-all duration-200 group relative"
                        >
                            {/* Top Row */}
                            <div className="flex items-start justify-between mb-4">
                                <div className="p-3 bg-violet-500/10 text-violet-600 dark:text-violet-400 rounded-lg group-hover:bg-violet-500 group-hover:text-white transition-colors">
                            ...
                            {/* Name */}
                            <h3 className="font-bold text-lg mb-3 text-foreground group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                                {tanda.nombre}
                            </h3>
                            ...
                            {/* hover arrow */}
                            <div className="absolute bottom-4 right-5 opacity-0 group-hover:opacity-100 transition-opacity text-violet-500">
